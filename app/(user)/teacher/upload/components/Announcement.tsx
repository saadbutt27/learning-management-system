...
const Announcement: React.FC<AnnouncementProps> = ({ courses, status }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/upload_announcement`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(`Failed to upload announcement. (${response.status})`);
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <input
          ...
          placeholder="Announcement topic here..."
          required
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          name="link"
          className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5"
          placeholder="A link e.g. https://example.com"
          pattern="https?://.*"
        />
      </div>
      ...
      <fieldset>
        <p className="block mb-2 text-base font-medium text-gray-900 dark:text-white">
          Select courses for this announcement
        </p>
